...
const PaymentPortal = () => {
  ...
  useEffect(() => {
    if (!registrationId) { navigate('/register'); return; }
    loadRegistration();
  }, [registrationId]);
  ...
  const handlePayNow = async () => {
    ...
    try {
      ...
      if (!razorpayLoaded) { alert("Failed to load Razorpay"); return; }
      ...
      const options = {
        ...
        handler: async function (rzpRes: any) {
          // Pass the data to the Confirm page to handle the final verification and ticket logic
          navigate('/confirm', { 
            state: { 
              registrationId: registration.id, 
              razorpayResponse: rzpRes 
            } 
          });
        },
        prefill: { name: registration.fullName, email: registration.email, contact: registration.phone },
        theme: { color: "#7c3aed" },
        modal: { ondismiss: () => setIsInitializing(false) }
      };
      ...
      <div className="max-w-xl w-full bg-zinc-900 border border-zinc-800 rounded-[3rem] overflow-hidden shadow-2xl relative">
        ...
        <div className="p-12">
          <header className="mb-10">
            ...
            <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest italic">Official Gateway Integration</p>
          </header>
          ...
          <div className="mb-8 p-4 bg-purple-600/10 border border-purple-500/20 rounded-2xl flex items-center gap-4">
             <div className="w-10 h-10 bg-purple-500/20 rounded-full flex items-center justify-center text-purple-400"><Award className="w-6 h-6" /></div>
             <p className="text-[10px] font-bold text-zinc-400 uppercase leading-relaxed">Secure SSL Encrypted Payment. Your data is protected.</p>
          </div>
          <button onClick={handlePayNow} disabled={isInitializing} className="w-full py-5 rounded-3xl font-black text-lg bg-white text-black uppercase tracking-widest shadow-2xl transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-3 disabled:opacity-50">
            <CreditCard className="w-6 h-6" /> Pay Now with Razorpay
          </button>
        </div>
      </div>
    </div>
